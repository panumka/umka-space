...
// --- Dynamic background tint from card colors ---------------------------------
(function () {
  const body = document.body;
  if (!body) return;

  const path = (location.pathname || "/").toLowerCase();
  let selector = ".release-card img";
  let fallback = [
    { r: 152, g: 86, b: 40 },
    { r: 78, g: 38, b: 24 },
    { r: 56, g: 30, b: 18 },
  ...
  if (path.startsWith("/panamabattle")) {
    selector = ".video-card img";
    fallback = [
      { r: 34, g: 96, b: 146 },
      { r: 30, g: 62, b: 102 },
      { r: 22, g: 48, b: 82 },
    ...
    selector = ".post-card img.thumb, .card img.thumb";
    fallback = [
      { r: 116, g: 64, b: 42 },
      { r: 72, g: 36, b: 28 },
      { r: 94, g: 48, b: 30 },
  ...
  const alpha = [0.36, 0.34, 0.24, 0.22];

  const setTints = (colors) => {
    const list = colors.slice(0, 4);
    while (list.length < 4) list.push(list[list.length - 1] || fallback[0]);
    list.forEach((c, i) => {
      body.style.setProperty(
        `--bg-tint-${i + 1}`,
        `rgba(${Math.round(c.r)}, ${Math.round(c.g)}, ${Math.round(c.b)}, ${alpha[i]})`
      );
    });
  };
  ...
  const sample = (img) => {
    ...
    try {
      ...
      return {
        r: Math.max(16, Math.min(220, r / count)),
        g: Math.max(16, Math.min(200, g / count)),
        b: Math.max(16, Math.min(185, b / count)),
      };
    } catch (err) {
      return null;
    }
  ...
  const compute = () => {
    const imgs = Array.from(document.querySelectorAll(selector)).slice(0, 16);
    if (!imgs.length) {
      setTints(fallback);
      return;
    }
    ...
    imgs.forEach((img) => {
      const c = sample(img);
      if (!c) return;
      if (colors.every((x) => distance(x, c) > 28)) colors.push(c);
    });
    setTints(colors.length ? colors : fallback);
  };

  // For visibility, force one tint toward dominant color families.
  const boostPalette = () => {
    const c1 = getComputedStyle(body).getPropertyValue("--bg-tint-1");
    if (c1 && /rgba?\(/.test(c1) && path.startsWith("/panamabattle")) {
      body.style.setProperty("--bg-tint-2", "rgba(26, 78, 128, 0.34)");
    }
  };
